...
final class InputReader[+In <: Input, +Config <: Reader.Config](
  ...
  private[this] val byteArrayInput: Input.FromByteArray   = new Input.FromByteArray(null)
  ...
  def tryReadStringCompare(utf8Bytes: Array[Byte]): Int = {
    if (_dataItem != DataItem.None) throw new IllegalStateException
    byteArrayInput.resetTo(0).byteArray = utf8Bytes
    val result = parser.tryReadStringCompare(byteArrayInput)
    if (result == 0 && (receiver ne receptacle)) receiver.onText(utf8Bytes)
    result
  }

  def tryReadBreakX(): Boolean = parser.tryReadBreak()
  ...
  /**
    * Skips the current/next (atomic) data item.
    *
  ...
  def skipElement(): this.type = {
    ...
    if (hasAnyOf(DI.Complex)) skipComplex(0) else clearDataItem()
  }
  ...
  @inline def clearDataItem(): this.type = {
    _dataItem = DataItem.None
    this
  }

  @inline private[borer] def rawParser: Receiver.Parser[_] = parser
